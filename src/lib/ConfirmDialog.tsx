export default function ConfirmDialog({open,msg,onConfirm,onCancel,C}: any){
  if(!open)return null;
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",zIndex:9000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={onCancel}>
      <div style={{background:C.bgCard,borderRadius:16,padding:28,maxWidth:360,width:"100%",border:"1px solid "+C.redBord,boxShadow:"0 24px 60px rgba(0,0,0,.6)"}} onClick={(e: any)=>e.stopPropagation()}>
